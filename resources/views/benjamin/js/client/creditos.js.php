...
/*  $scope.carts_clicado = $firebaseArray(ref_carts_targer);
 ...
 // alert(comanda);
  ...
  $scope.modal_credito = function($valor) {
    // body...
    $('#modal_produto').modal('show');
    $scope.credito_selecionado = $valor;
    //$scope.add_credito($valor);
  }


  $scope.add_credito = function ($valor) {
    // body...
    //alert("Vai comprar creditos");
    
    var ref_carts_targer = firebase.database().ref().child("users/"+$scope.logado_id+"/vouchers");
    $scope.voucher = $firebaseArray(ref_carts_targer);
    $scope.voucher.$add({
      comanda : 'voucher' ,
      shop: <?php echo $loja;?>,
      user : $scope.logado_id,
